...
export const OrderCard = ({ orderId, date, initialStatus }: OrderCardProps) => {
  const {
    ...
    orderStats,
    loading,
  } = useOrderDetail(orderId.toString());
  ...
  const orderTitle = mainProduct
    ? `${mainProduct.name} ${
        uniqueProductsCount > 0
          ? `y ${uniqueProductsCount} productos más...`
          : ""
      }`
    : `Pedido #${orderId}`;
  ...
  const statusInfo = getStatusConfig(
    orderDetail?.order?.status || initialStatus
  );
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col gap-1">
            ...
            <div className="mt-1">
              <span
                className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border inline-block ${statusInfo.color}`}
              >
                {statusInfo.label}
              </span>
            </div>
        ...
        <div className="text-right pl-4 border-l border-gray-100 sm:border-l-0 sm:pl-0 pt-2 sm:pt-0 border-t sm:border-t-0 mt-2 sm:mt-0 w-full sm:w-auto flex justify-between sm:block items-center">
          <span className="sm:hidden text-gray-500 text-sm font-medium">
            Total:
          </span>
            ...
            {loading ? (
              ...
              <>
                <p className="text-xl font-bold text-blue-600">
                  ${orderStats.totalMoney.toLocaleString("es-CO")}
                </p>
                <p className="text-xs text-gray-500 font-medium">
                  {orderStats.totalItems}{" "}
                  {orderStats.totalItems === 1 ? "artículo" : "artículos"}
                </p>
              </>
            )}
          </div>
        ...
        <div className="border-t border-gray-100 bg-gray-50 p-4 animate-fade-in-down">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded bg-white overflow-hidden flex-shrink-0 border border-gray-200 flex items-center justify-center relative">
                      {product.img ? (
                        <Image
                          src={product.img}
                          alt={product.name}
          ...
          <div className="mt-4 pt-3 border-t border-gray-200 flex justify-between items-center">
            <span className="text-sm text-gray-500">
              Gastos de envío e impuestos incluidos
            </span>
            <span className="text-lg font-bold text-blue-600">
              Total: ${orderStats.totalMoney.toLocaleString("es-CO")}
            </span>
          </div>
        </div>
      )}
